...
class LogRoomController extends Controller
{
    use ModelForm;
    ...
    protected function grid($id)
    {
        ...
        return Admin::grid(Radacct::class, function (Grid $grid) use ($user) {
            ...
            if ($user->check_in && $user->check_out) {
                $grid->model()->whereBetween('acctstarttime', [$user->check_in, $user->check_out]);
            } elseif ($user->check_in) {
                $grid->model()->where('acctstarttime', '>=',$user->check_in);
            } elseif ($user->check_out) {
                $grid->model()->where('acctstarttime', '<=',$user->check_out);
            }
